...
<form method="POST" action="{{ route('transactions.update', $transaction->id) }}" class="space-y-4">
    ...
    <div>
        ...
        <input type="text" name="serial_number" value="{{ $transaction->serial_number }}" class="w-full border px-4 py-2 rounded">
    </div>
    <div>
        <label class="block text-gray-700">Product Name</label>
        <input type="text" name="product_name" value="{{ $transaction->product_name }}" class="w-full border px-4 py-2 rounded">
    </div>
    <div>
